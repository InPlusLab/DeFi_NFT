...
def getTxs_fromDefi():
    ...
    outputPath_dict="/mnt/sde1/peilin_defi/data/acrossXblock/txTypeMap/tx_nft.map"
    txMap={}
...
def combineMap():
    inputPath_dict="/mnt/sde1/peilin_defi/data/acrossXblock/txTypeMap/tx_nft.map"
    nft_txMap={}
    with open(inputPath_dict, "rb") as tf:
        nft_txMap=pickle.load(tf)
        
    inputPath_dict="/mnt/sde1/peilin_defi/data/acrossXblock/txTypeMap/tx_defi.map"
    defi_txMap={}
    with open(inputPath_dict, "rb") as tf:
        defi_txMap=pickle.load(tf)
        
    
    for key,value in nft_txMap.items():
        if key not in defi_txMap:
            defi_txMap[key]=value
        else:
            defi_txMap[key].extend(value)


    with open("/mnt/sde1/peilin_defi/data/acrossXblock/txTypeMap/tx_defiNft.map", "wb") as tf:
        pickle.dump(defi_txMap,tf)   
# ··············································································
...
def getTxsData_fromXblock():
    inputPath_dict="/mnt/sde1/peilin_defi/data/acrossXblock/txTypeMap/tx_defiNft.map"
    txMap={}
    with open(inputPath_dict, "rb") as tf:
    ...
    for fileName in os.listdir(dir):
        ...
        outputPath_csv="/mnt/sde1/peilin_defi/data/acrossXblock/normalTransaction/defiNft/"+fileName.split(".")[0]+".csv"
